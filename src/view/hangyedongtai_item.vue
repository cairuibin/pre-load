<template>
  <div class="wrap212">
    <div class="img_qiyezixun container">
      <img src="../assets/cimg/hangyedongcha/hangyedongcha.jpg" alt="">
    </div>
    ...
    <div class="footer_page">
      ...
      <span
        v-for="(v,i) in Math.ceil(hangyedongcha_itemlist.length/pageSize)"
        :key="i"
...
export default {
  ...
  props: ["i", "pid"],
  computed: {
    ...mapState(["hangyedongcha_itemlist"]),

    curArr() {
      return this.hangyedongcha_itemlist.slice(
...
this.$router.back(-1)
    ...
    godetail(v) {
      console.log(v);
       window.sessionStorage.setItem('hangyedongcha_iteml',JSON.stringify(v))
      this.GoTop();
      
      this.$router.push(`/hangyedongtaifor`);
    },
    next() {
      console.log(this.pageNum, Math.ceil(this.hangyedongcha_itemlist.length / this.pageSize))
      if (
        this.pageNum < Math.ceil(this.hangyedongcha_itemlist.length / this.pageSize)
      ) {
    ...
    pre() {
      if (
        ...
        this.pageNum = Math.ceil(
          this.hangyedongcha_itemlist.length / this.pageSize
        );
      }
    },
    clikitem(i) {
      if (i < Math.ceil(this.hangyedongcha_itemlist.length / this.pageSize)) {
        this.pageNum = i + 1;
      }
    },
  },
